...
function Register() {
	...
	const formik = useFormik({
		...
		validationSchema: yup.object({
			username: yup.string().required("שם משתמש הוא חובה").min(3, "שם משתמש חייב להיות לפחות 3 תווים").max(20, "שם משתמש לא יכול להיות ארוך מ-20 תווים"),
			...
			password: yup
				...
				.min(8, "סיסמא חייבת להיות לפחות באורך 8 תווים")
				.matches(/^(?=.*[A-Z])(?=.*[a-z])(?=(?:.*\d){4,})(?=.*[)!@%$#\^&*\-_\(\)]).*$/, "הסיסמה חייבת להכיל לפחות אות אחת גדולה, אות קטנה, 4 ספרות, ותו מיוחד"),
		}),
